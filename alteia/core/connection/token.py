...
Token = collections.namedtuple("Token", ("access_token", "token_type", "expires_in", "refresh_token"))


class TokenManager:
    def __init__(self, *, connection, credentials, access_token=None, token_type=None):
    ...
    def renew_token(self):
        ...
        headers = {
            "Authorization": f"Basic {self._credentials.encoded_secret}",
            "Content-Type": "application/json",
        }
        data = json.dumps(self._credentials.data)
        decoded_data = self._connection.post(path=self._path, data=data, headers=headers, as_json=True)

        if "access_token" in decoded_data:
            self._token = Token._make(
                (
                    decoded_data["access_token"],
